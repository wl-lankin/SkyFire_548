...
bool ArenaTeam::AddMember(uint64 playerGuid)
{
    ...
    SF_LOG_DEBUG("bg.arena", "Player: %s [GUID: %u] joined arena team type: %u [Id: %u, Name: %s].", playerName.c_str(), GUID_LOPART(playerGuid), GetType(), GetId(), GetName().c_str());
...
bool ArenaTeam::LoadMembersFromDB(QueryResult result)
{
    ...
    do
    {
        ...
        {
            SF_LOG_ERROR("sql.sql", "ArenaTeam %u has member with empty name - probably player %u doesn't exist, deleting him from memberlist!", arenaTeamId, GUID_LOPART(newMember.Guid));
    ...
    if (Empty() || !captainPresentInTeam)
    {
        ...
        SF_LOG_DEBUG("bg.arena", "ArenaTeam %u does not have any members or its captain is not in team, disbanding it...", TeamId);
        return false;
    }
...
void ArenaTeam::SetCaptain(uint64 guid)
{
    ...
    if (Player* newCaptain = ObjectAccessor::FindPlayer(guid))
    {
        ...
        if (oldCaptain)
        {
            SF_LOG_DEBUG("bg.arena", "Player: %s [GUID: %u] promoted player: %s [GUID: %u] to leader of arena team [Id: %u, Name: %s] [Type: %u].",
...
void ArenaTeam::DelMember(uint64 guid, bool cleanDb)
{
    ...
    if (Player* player = ObjectAccessor::FindPlayer(guid))
    {
        ...
        SF_LOG_DEBUG("bg.arena", "Player: %s [GUID: %u] left arena team type: %u [Id: %u, Name: %s].", player->GetName().c_str(), player->GetGUIDLow(), GetType(), GetId(), GetName().c_str());
    }
